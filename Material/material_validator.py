...
class MATERIAL_OT_clear_results(Operator):
    """清除验证结果"""
    bl_idname = "material.clear_validation_results"
    bl_label = "清除结果"
    bl_description = "清除所有验证结果"
    bl_options = {'REGISTER', 'UNDO'}
    
    def execute(self, context):
        context.scene.material_validator_results.clear()
        self.report({'INFO'}, "已清除所有验证结果")
        return {'FINISHED'}
...
class MATERIAL_PT_connection_validator(Panel):
    ...
    def draw(self, context):
        ...
        if scene.material_validator_results:
            ...
            # 清除结果按钮
            row.operator("material.clear_validation_results",
                        text="清除结果",
                        icon='TRASH')
